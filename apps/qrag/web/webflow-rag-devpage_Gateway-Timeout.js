...
var fileInfoPageBody = 'webflow-rag-devpage.js  2-19 1109 update for gateway timeout';
...
function submitInputRag(event) {
    ...
    const botContainer = document.getElementById(`container_${trailingId}`);
    const errorElement = botContainer.querySelector('.botsubmit-error');
    const answerDisplay = botContainer.querySelector('.answer-display');
    ...
    try {
        ...
        submitIcon = document.getElementById(submitIconId);
        rawUserQuestion = document.getElementById(userInputId).value.trim();
        ...
        setButtonToLoadingState(submitButton, submitIcon);
        ...
        .catch(error => {
            ...
            // Get the answer display element
            if (answerDisplay) {
                if (error.message === 'GATEWAY_TIMEOUT') {
                    answerDisplay.innerHTML = `
                        <div class="error-message" style="color: #d93025; padding: 10px; border: 1px solid #d93025; border-radius: 4px; margin: 10px 0;">
                            <p><strong>Sorry - the AI took too long to respond.</strong></p>
                            <p>Please:</p>
                            <ol>
                                <li>Copy your question</li>
                                <li>Refresh this page</li>
                                <li>Paste your question back in and try again</li>
                            </ol>
                            <p>If the problem persists, try asking a simpler question or contact contact@focusonfoundations.org.</p>
                        </div>`;
                } else {
                    answerDisplay.innerHTML = `
                        <div class="error-message" style="color: #d93025; padding: 10px; border: 1px solid #d93025; border-radius: 4px; margin: 10px 0;">
                            <p><strong>Sorry - an error occurred.</strong></p>
                            <p>Please try again later or contact contact@focusonfoundations.org if the problem persists.</p>
            ...
            setButtonToStartState(submitButton, submitIcon);
        ...
        console.error('submitInputRag - Error:', error);
        logErrorToMonitoring(error, 'submitInputRag function');
        
        if (errorElement) {
            errorElement.style.display = 'block';
            errorElement.innerHTML = 'An unexpected error occurred. Please try again.';
            setTimeout(() => { errorElement.style.display = 'none'; }, 3000);
        }
        
        // Reset button state if we have the button reference
        if (submitButton && submitIcon) {
            setButtonToStartState(submitButton, submitIcon);
        }
        return false;
    }
...
function qragLLM(routingJsonData, large_context_filename) {
    ...
    .catch(error => {
        ...
        if (error.message === 'Failed to fetch' || error.message === 'GATEWAY_TIMEOUT') {
            throw new Error('GATEWAY_TIMEOUT');
        }
        throw error;
    });
