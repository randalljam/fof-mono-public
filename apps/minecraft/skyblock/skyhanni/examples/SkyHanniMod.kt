// SOURCE: https://github.com/hannibal002/SkyHanni/blob/beta/src/main/java/at/hannibal2/skyhanni/SkyHanniMod.kt
//
// This is the MAIN MOD OBJECT — the central singleton that holds all state.
// Key things to notice:
//   - It's a Kotlin `object` (singleton), not a class
//   - Annotated with @SkyHanniModule so it's auto-registered
//   - Holds references to config, storage, data objects
//   - Registers the /sh command using Brigadier
//   - Uses @HandleEvent for tick processing and shutdown hooks
//   - Uses coroutines (CompatCoroutineManager) for async work
...
object SkyHanniMod : CompatCoroutineManager by SkyHanniCoroutineManager(
    ...
    fun CoroutineSettings.async(block: suspend CoroutineScope.() -> T): Deferred<T> =
        with(SkyHanniMod) { asyncCoroutine(block) }

    fun CoroutineSettings.asyncUnScoped(block: suspend () -> T): Deferred<T> =
