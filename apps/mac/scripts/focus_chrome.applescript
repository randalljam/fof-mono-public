on run argv
    ...
    if running of application "Google Chrome" is false then return "APP_NOT_RUNNING"
    ...
    tell application "Google Chrome"
        ...
        repeat with windowIndex from 1 to count windows
            ...
            repeat with tabIndex from 1 to count tabs of browserWindow
                ...
                if targetTitle is not "" then set titleMatches to (title of browserTab as text) is targetTitle
        ...
        set active tab index of window (targetWindowIndex as integer) to (targetTabIndex as integer)
        try
            set minimized of window (targetWindowIndex as integer) to false
        end try
