# Stellar Transcriber Specification

## Purpose
Stellar Transcriber is an S1-dev Python CLI app for evaluating and improving diarized transcript quality across Focus on Foundations corpora. It catalogs raw/reference transcript pairs from S3 manifests, fetches local eval files, scores raw and draft transcripts with config-driven metrics, and experiments with deterministic and LLM repairs for speaker-boundary alignment. The current app is developer-facing and works through scripts, gitignored local data, reference reports, and shared `core/` modules rather than a deployed UI.

## Workflows
### Workflow: Build the evaluation corpus
run inventory -> fetch paired transcript markdown from S3 -> load corpus profile config -> run baseline eval -> review score summaries and artifacts.
Exercises requirements: Corpus Inventory Catalog, Evaluation Pair Fetching, Corpus Evaluation Profiles, Baseline Transcript Evaluation, Transcript Scoring Metrics.

### Workflow: Create draft transcripts
choose local raw transcript inputs -> run the draft build CLI -> apply deterministic cleanup or LLM repair -> produce single or dual draft markdown for inspection.
Exercises requirements: Draft Build CLI, Deterministic Draft Cleanup, LLM Draft Processing, Dual Transcript Merging.

### Workflow: Measure alignment repair quality
build a defect fixture or supply real raw/reference paths -> run the alignment ladder -> inspect per-episode error counts -> aggregate matching ladder reports.
Exercises requirements: Alignment Fixture Generation, Alignment Ladder Evaluation, Alignment Report Aggregation, Transcript Scoring Metrics.

### Workflow: Compare and preserve M3B dual-LLM runs
select resolvable episodes -> run M3B scoring phases with or without LLM calls -> compare phase reports -> build a five-file review bundle.
Exercises requirements: M3B Dual-LLM Scoring Runner, M3B Review Bundle Archiving, Dual Transcript Merging.
