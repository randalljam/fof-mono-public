...
## Purpose
Math Quiz is a local-first browser application for assessing and building children's arithmetic fluency, especially single-digit addition, using fast keypad practice, per-learner SQLite history, analysis dashboards, and motivational game wrappers. The active `anchor.html` flow runs on the local math-quiz dev server so it can load, save, and back up learner files; older quiz, fluency, and analysis pages remain available for legacy sessions and review.
## Workflows
### Workflow: Kid Starts Practice
Open `anchor.html` -> pick a learner -> load that learner's latest SQLite file -> choose Fluency feast, Targeted practice, Problem list, or Quick quiz -> answer with the keypad -> save or abandon the run.
Exercises requirements: Kid Landing and Setup, Learner File Lifecycle, Anchor Quiz Runs, Fluency Computation, Fluency-Generated Problem Lists, Targeted Practice, Problem Lists and Quick Quizzes.

### Workflow: Coach Reviews and Tunes
Open analysis for a learner file -> inspect response-time or fluency heatmaps -> filter by operation, flags, category, and session window -> edit flags or rubric thresholds -> use tracker/list tools to prepare the next practice.
Exercises requirements: Analysis Dashboard, Fluency Tracker Dashboard, Fluency Computation, Fluency-Generated Problem Lists, Problem Lists and Quick Quizzes, Canonical Session Data.

### Workflow: Local File Lifecycle
Start the dev server -> list folders and learner files -> load the latest learner SQLite -> save a new run -> archive the single-session file, update the accumulated file, and refresh generated quick-practice/config data.
Exercises requirements: Learner File Lifecycle, Canonical Session Data, Problem Lists and Quick Quizzes, Targeted Practice.

### Workflow: Legacy Quiz and JSON Review
Open `math_quiz.html` -> run a preset, custom, uploaded-list, or speech-assisted quiz -> save browser-local JSON -> load the captured data into analysis or fluency tracker pages.
Exercises requirements: Legacy Quiz Flow, Canonical Session Data, Analysis Dashboard, Fluency Tracker Dashboard.

### Workflow: Dragon Motivation Loop
Start the math-quiz dev server -> open `dragon/index.html` -> pick a learner -> complete Fluency Feast bursts -> save through `/api/save-run` -> unlock dragon milestones and story beats -> parent observes or sends letters from Game Master.
Exercises requirements: Dragon Fluency Game, Dragon Story and Game Master, Fluency Computation, Learner File Lifecycle, Canonical Session Data.
## Requirements
