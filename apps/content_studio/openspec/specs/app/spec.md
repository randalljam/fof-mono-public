...
## Purpose
Content Studio is a Python media-generation app for creating animations, video clips, speech, music, sound effects, and dragon-focused 3D assets from verbal descriptions or source images. Its core behavior is a closed generate -> inspect -> retry loop that rejects broken visual or speech outputs before a result is treated as shippable. This single `app` capability is the S1-dev baseline for the whole application; split it later only when the behavior surface is too large for one readable spec.
## Workflows
### Workflow: Verified Visual Clip Generation
Run `animate` or `video` -> build an animation/video request -> choose a provider -> generate candidates -> sample frames and apply visual policy -> retry with stronger negative prompt when needed -> materialize the chosen clip.
Exercises requirements: Media Request Types, Provider Registry And Dispatch, Generation Providers, Command-Line Generation And Verification, Verified Generation Loop, Visual Verification Policy, Media File Handling, Dependency-Light Testable Boundaries

### Workflow: Verified Audio Generation
Run `audio` -> build a speech/music/sfx request -> choose a provider -> generate candidates -> verify speech by transcript or music/sfx structurally -> materialize the chosen audio output.
Exercises requirements: Media Request Types, Provider Registry And Dispatch, Generation Providers, Command-Line Generation And Verification, Verified Generation Loop, Audio Verification Policy, Media File Handling, Dependency-Light Testable Boundaries

### Workflow: Existing Media Verification
Run `verify` for an existing file -> classify it as speech or visual media -> run the matching verifier -> print a JSON verdict and use the pass/fail exit code.
Exercises requirements: Command-Line Generation And Verification, Visual Verification Policy, Audio Verification Policy, Media File Handling

### Workflow: Consumer Asset Profile Delivery
Read a tracked consumer profile -> place references under `_data/profiles/<slug>/reference/` -> generate and verify staging media -> move passing outputs to `approved/` -> deliver approved assets into the consumer app.
Exercises requirements: Consumer Asset Profiles, Command-Line Generation And Verification, Verified Generation Loop, Visual Verification Policy, Audio Verification Policy, Media File Handling

### Workflow: Dragon 3D Asset Build
Run model3d `build` -> generate or reuse a Rodin/Meshy mesh -> rig and animate it in Blender -> validate the GLB contract -> render previews and report the summary.
Exercises requirements: Model3D Mesh Generation, Model3D Blender Pipeline, Dragon GLB Validation
## Requirements
