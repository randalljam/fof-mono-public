...
def count_tokens(input_string):  # no unittests
    ...
    Falls back to a chars/4 estimate when the encoding file cannot be loaded
    (e.g. network-restricted cloud environments block the tiktoken download).
    ...
    if _TIKTOKEN_ENCODING is None and not _TIKTOKEN_UNAVAILABLE:
        try:
            _TIKTOKEN_ENCODING = tiktoken.get_encoding('cl100k_base')
        except Exception as exc:
            _TIKTOKEN_UNAVAILABLE = True
            warnings.warn(f"tiktoken encoding unavailable ({exc}); count_tokens falling back to chars/4 estimate")
    if _TIKTOKEN_UNAVAILABLE:
        return max(1, len(input_string) // 4) if input_string else 0
    token_count = len(_TIKTOKEN_ENCODING.encode(input_string))
    return token_count
### Model pricing (loaded from core/llm_model_prices.json)
_LLM_PRICES_FALLBACK = {
...
def load_llm_model_prices(prices_path=None):
    """Load model token prices from JSON; return models dict."""
    if prices_path is None:
        prices_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'llm_model_prices.json')
    if not os.path.isfile(prices_path):
        return dict(_LLM_PRICES_FALLBACK)
    ...
    if not models:
        return dict(_LLM_PRICES_FALLBACK)
    return models
def reload_token_price_dict():
    """Reload TOKEN_PRICE_DICT from llm_model_prices.json."""
    global TOKEN_PRICE_DICT
    TOKEN_PRICE_DICT = load_llm_model_prices()
    return TOKEN_PRICE_DICT
TOKEN_PRICE_DICT = load_llm_model_prices()
def compute_cost_from_tokens(input_tokens, output_tokens, model, token_price_dict=None, cached_input_tokens=0):
    """Return dict with input_cost, output_cost, total_cost in USD."""
    token_price_dict = token_price_dict or TOKEN_PRICE_DICT
    if model not in token_price_dict:
        raise ValueError(f"Model {model} not found in token price dictionary.")
    prices = token_price_dict[model]
    non_cached = max(0, input_tokens - cached_input_tokens)
    input_cost = (non_cached / 1_000_000) * prices['input_token_price']
    if cached_input_tokens:
        cached_rate = prices.get('cached_input_token_price', prices['input_token_price'])
        input_cost += (cached_input_tokens / 1_000_000) * cached_rate
    output_cost = (output_tokens / 1_000_000) * prices['output_token_price']
    return {
...
class LlmUsageAccumulator:
    """Accumulate LLM API usage and cost across multiple calls."""
    def __init__(self, model, token_price_dict=None):
        self.model = model
        self.token_price_dict = token_price_dict or TOKEN_PRICE_DICT
        self.input_tokens = 0
    ...
    def add_usage(self, input_tokens, output_tokens, cached_input_tokens=0, is_retry=False):
    ...
    def summary(self):
        costs = compute_cost_from_tokens(
            self.input_tokens, self.output_tokens, self.model, self.token_price_dict,
...
def extract_usage_from_openai_response_json(response_json):
    """Extract input/output token counts from OpenAI chat completion JSON."""
...
def extract_usage_from_anthropic_response(response):
    """Extract token counts from Anthropic Message response."""
...
def openai_function_call(prompt, content, tools, model=OPENAI_MODEL, verbose=False, temperature=None):
    ...
    try:
        ...
        return None
def openai_function_call_with_usage(prompt, content, tools, model=OPENAI_MODEL, verbose=False, temperature=None):
    """Like openai_function_call but also returns usage dict from the API response."""
    try:
        messages = [
            {"role": "system", "content": prompt},
            {"role": "user", "content": content}
        ]
        request_params = {"messages": messages, "model": model}
        if temperature is not None:
            request_params["temperature"] = temperature
        if tools and len(tools) > 0:
            request_params["tools"] = tools
        ...
        if response and response.status_code == 200:
            response_json = response.json()
            usage = extract_usage_from_openai_response_json(response_json)
            message = None
            if "choices" in response_json and len(response_json["choices"]) > 0:
                message = response_json["choices"][0].get("message", {})
            return message, usage, response_json
        return None, {'input_tokens': 0, 'output_tokens': 0, 'cached_input_tokens': 0}, None
    except Exception as e:
        if verbose:
            print(f"Error in function call: {str(e)}")
        return None, {'input_tokens': 0, 'output_tokens': 0, 'cached_input_tokens': 0}, None
...
### ANTHROPIC LLM
def anthropic_chat_completion_request(messages, model=ANTHROPIC_MODEL, system=None, tools=None, tool_choice=None, max_tokens=4096, temperature=0.7):
    """
    ...
    :param tool_choice: optional dict forcing tool use, e.g. {"type": "tool", "name": "my_tool"}
...
def anthropic_function_call(prompt, content, tools, model=ANTHROPIC_MODEL, verbose=False):
    ...
    try:
        ...
        if verbose:
            print(f"Error in function call: {str(e)}")
        return None
def anthropic_function_call_with_usage(prompt, content, tools, model=ANTHROPIC_MODEL, verbose=False):
    """Like anthropic_function_call but also returns usage dict."""
    try:
        ...
        if verbose:
            print(f"Error in function call: {str(e)}")
        return None, {'input_tokens': 0, 'output_tokens': 0, 'cached_input_tokens': 0}
...
def create_simple_llm_file(file_path, prompt, suffix_new, mode, split_file_function, provider="openai", *args, **kwargs):
    ...
    :return: string of the path to the file with the updated content
    """
    from core.fileops import delete_file
...
def create_qa_file_from_transcript_incremental(file_path, fcall_prompt):
    ...
    """
    from core.structured import count_blocks
...
def create_qa_file_from_transcript_sections(file_path, fcall_prompt, provider="openai", heading="### transcript", delimiter='---'):
    ...
    log_lines.append("function: core.llm.create_qa_file_from_transcript_sections")
    ...
    log_lines.append("prep: core.llm.add_transcript_section_delimiters for non-FDA speakers")
...
def get_questions_from_qa_file(qa_file_path, heading):
    ...
    """
    from core.structured import get_blocks_from_file, get_field_value
    from core.fileops import get_heading, get_heading_level
...
def get_questions_from_extract_log(qa_file_path, heading_level=5, verbose=False):
    ...
    """
    from core.fileops import get_heading_level
...
def generate_and_save_question_embeddings(qa_file_path, questions, verbose=True):
    ...
    """
    import numpy as np
    from core.vectordb import generate_embedding
...
def create_extract_log_header(fcall_prompt, provider, model, round_num, round_name):
    ...
    if round_num == 1:
        ...
        log_lines.append(f"datetime: {datetime}")
        #log_lines.append("source file prep: core.llm.add_transcript_section_delimiters for non-FDA speakers")
...
def evaluate_qa_extraction(transcript, qa_file_path):
    ...
    """
    from core.structured import get_all_fields_dict, count_blocks
...
### DENOVO TRANSCRIPT CLEANUP
PROMPT_DENOVO_SINGLE_V1 = """You are a transcript cleanup assistant. Fix speaker-segmentation defects in diarized transcripts:
- Broken sentences split across speaker transitions (move errant boundary text to the correct segment).
- Wrong speaker attribution when clearly incorrect.
- Obvious ASR artifacts (partial words, false starts) without changing meaning.

Preserve timestamps unless clearly wrong. Return the corrected segment list via the function call.
Do not invent content. Keep speaker labels as given unless attribution is clearly wrong."""
PROMPT_DENOVO_DUAL_V1 = """You are a transcript arbitration assistant. Two ASR models transcribed the same audio.
Given both versions of a disputed island (segment range), return the best consensus segment list:
- Move errant boundary text across transitions.
- Insert segments present in only one source when clearly correct.
- Pick or blend dialogue on disagreement. Resolve speaker attribution.

Preserve timestamps from the stronger source when possible. Return via the function call."""
PROMPT_DENOVO_SINGLE_V2 = """You are a conservative second-pass assistant for diarized transcript SEGMENTATION only. The input has already been through deterministic cleanup (_draftds). Do not redo obvious boundary-fragment repairs that deterministic code can handle. Your job is only to handle rare context-heavy cases that are hard for deterministic rules.

Use the LLM only for:
1. Missed speaker split inside one segment: one segment clearly contains two adjacent speaker turns, and the text itself gives a strong boundary.
2. Whole-segment speaker label correction: a complete segment is assigned to the wrong speaker, and the surrounding question/answer context makes that overwhelmingly clear.
3. No-op judgment: if the case is ambiguous, return the input segments unchanged.

Hard rules:
- Do NOT copyedit. Do not paraphrase, improve grammar, remove duplicated words, remove fillers, normalize punctuation, fix punctuation, or make the transcript read better.
- Every input word must appear exactly once in the output, in the same order, except read-only context blocks must not be included.
- Preserve each timestamp string verbatim. Never invent, reformat, or alter timestamps.
- Do not merge segments. Do not delete spoken words. Only split a segment or change a speaker label when evidence is overwhelming.
- The 'context before' and 'context after' blocks are read-only orientation; do not include them in the output and do not move words into or out of them.
- Speaker-label changes require stronger evidence than boundary-word moves. You have no audio, so do not guess from topic, expertise, writing style, or which speaker "sounds likely."
- If overlap/crosstalk, duplicated words, false starts, or ASR weirdness makes the case uncertain, preserve the input structure for human review.

Examples:

1. Missed speaker split inside one segment. Input:
  Speaker A  0:10
  I agree with that. What do you think about the result? Well, I was surprised by it.
Corrected output:
  Speaker A  0:10
  I agree with that. What do you think about the result?
  Speaker B  0:10
  Well, I was surprised by it.
Why: the second sentence is a direct answer to Speaker A's question, so this is a likely missed speaker change. The split-off part reuses the original timestamp.

2. Whole-segment speaker label correction. Input:
  Speaker A  1:00
  Do you think pain is always bad?
  Speaker A  1:05
  No, I think pain can be information.
  Speaker B  1:12
  That's interesting.
Corrected output:
  Speaker A  1:00
  ...
  Speaker B  1:12
  That's interesting.
Why: Speaker A asks a question, and the next segment is a direct answer before Speaker B continues. This is strong transcript-context evidence.

3. Possible speaker-label change, but evidence is not strong enough. Input:
  Speaker A  2:00
  Do you think this matters for the theory?
  Speaker A  2:05
  I think it matters only if the theory predicts something different.
  Speaker B  2:12
...
Corrected output:
  ...
  Speaker B  2:12
  Right, that's the distinction I was trying to make.
Why: a speaker can answer or refine their own question. Do not relabel the middle segment just because it follows a question.

4. Possible split inside a segment, but evidence is too weak. Input:
  Speaker A  3:00
  I was thinking about that problem. Maybe there are two explanations. The first is about incentives. The second is about knowledge.
  Speaker B  3:20
  Okay.
Corrected output:
  Speaker A  3:00
  ...
  Speaker B  3:20
  Okay.
Why: the segment contains multiple sentences, but no clear speaker-change cue. Do not split just because the segment is long.

Return the corrected segment list via the function call."""
PROMPT_DENOVO_DUAL_V2 = """You are a conservative transcript SEGMENTATION arbitration assistant. Two ASR systems (A and B) transcribed the same audio; you are given both versions of a disputed span between agreed anchor points. The versions may disagree on segmentation, speaker attribution, missing/dropped speech, duplicated speech, or wording. Produce the best safe consensus segment list.

Primary goals:
- Prefer the segmentation that respects true speaker turns: no segment should end mid-sentence with its continuation at the start of the next segment, and each turn should carry the correct speaker when transcript context makes that clear.
- Include dialogue present in only one version when it is plausibly real speech the other system dropped.
- Avoid duplicating the same speech from both versions.
- When A and B contain the same underlying speech span with different speaker labels, timestamps, punctuation, or wording, choose one consensus version of that span. Do not output both copies.
- When speaker numbers differ between A and B, choose the speaker label that best fits the local speaker continuity and turn-taking in the selected consensus. Speaker numbers are model-local guesses, not stable identities across systems.
- Prefer the version that keeps short answers, acknowledgements, and question responses as separate speaker turns when context supports them.
- If the versions agree, return that shared segment list unchanged.

Guardrails:
- This is still segmentation arbitration, not copyediting. Do NOT paraphrase, summarize, fix grammar, remove fillers, improve punctuation, remove duplicated words, or improve wording.
- On wording disagreement, pick the more plausible existing reading from A or B. Do not paraphrase, and do not output words absent from both versions. Do not compose a polished third version unless it is only the result of moving unchanged words between segment boundaries.
- Timestamps: copy each output timestamp verbatim from version A when the consensus segment follows A's boundaries, otherwise from B. Never invent or reformat timestamps.
- Keep speaker label style consistent with the selected source segment and the input style.
- Speaker attribution can be adjusted only when context is overwhelming. You have no audio, so do not infer a speaker from content expertise, topic, or style alone.
- If the inputs use real speaker names, preserve the real name that best matches the local turn. If the inputs only use numbered labels (`Speaker 1`, `Speaker 10`), choose the numbered label that best preserves local speaker continuity; do not invent speaker names.
- If both versions are messy or require audio judgment, choose the safer existing segmentation and leave wording unchanged.
- Duplicate-span guard: if version A and version B cover the same timestamp range or the same dialogue content, choose one consensus version. Do not output both copies, even if the speaker numbers differ. Never append A's copy followed by B's copy for the same underlying speech.

Examples:

1. If version A and B both contain messy ASR wording with punctuation differences, choose one version and do not polish it. Version A:
  Speaker A  0:10
  And so, so, so you asked me how do you know how do you sorry. How do you know what?
Version B:
  Speaker A  0:10
  And so so so you asked me, how do you know? How do you, sorry, how do you know what?
Safe consensus:
  Speaker A  0:10
  And so, so, so you asked me how do you know how do you sorry. How do you know what?
Either version's wording is acceptable, but do not create a third polished version.

2. If a short answer is echoed by the next speaker, prefer the version that keeps the short answer as its own turn. Version A:
  Speaker A  3:00
  How do you know that withdrawing from the pain is what causes?
  Speaker B  3:05
  Isn't yeah
  Speaker A  3:06
  isn't yeah. Yes. How do you know that withdrawing from the pain isn't what causes the suffering
Version B:
  Speaker A  3:00
  How do you know that withdrawing from the pain is what causes?
  Speaker B  3:05
...
3. If neither version clearly resolves overlap/crosstalk, choose the safer existing version unchanged. Version A:
  Speaker A  4:00
  Yes. But if it did
  Speaker B  4:02
  Whereas
  ...
  by some way if it
  Speaker B  4:03
  did, then if it did, then we would still observe it.
Version B:
  Speaker A  4:00
  Yes. But if it did Whereas by some way if it
  Speaker B  4:03
  did, then if it did, then we would still observe it.
Safe consensus:
  Speaker A  4:00
  ...
  Speaker B  4:03
  did, then if it did, then we would still observe it.
Why: Version A preserves the uncertain overlap for human review; do not invent a smoother sentence.

4. If A and B contain the same audience-question span with different speaker numbers, output it once. Version A:
  Speaker 1  59:22
  Yeah. Let me talk to her and I'll get back to you.
  Speaker 0  59:30
  Fair enough. I see a question from the audience.
  Speaker 7  59:43
  I wanted to let you know there was an accident last night. That's my question. Thank you.
Version B:
  Speaker 2  59:22
  Yeah let me talk to her and I'll get back to you.
  Speaker 0  59:30
...
Safe consensus:
  ...
  Speaker 7  59:43
  I wanted to let you know there was an accident last night. That's my question. Thank you.
Why: these are duplicate copies of the same underlying speech. Choose one consensus version and do not output both copies.

Speaker-label note: In this example, `Speaker 1` and `Speaker 2` may be the same person across the two ASR systems, and `Speaker 7` and `Speaker 10` may be the same audience member. Speaker numbers differ across raw transcripts. Choose the label from the selected consensus source or the label that best preserves local speaker continuity. Do not invent speaker names unless real names already appear in the input.

Return via the function call."""
PROMPT_DENOVO_DUAL_V3 = """You are a conservative transcript SEGMENTATION arbitration assistant. Two ASR systems (A and B) transcribed the same audio. You are given both versions of ONE SHORT DISPUTED SPAN. The speech immediately before and after this span is agreed between the two systems (shown as read-only context when available), so the disputed span covers the same audio in both versions. The versions may disagree on where speaker turns start and end, speaker attribution, speech one system dropped, or wording.

Return the best safe consensus segment list for the disputed span only. Never re-output the read-only context.

How to choose:
- Prefer the version whose speaker turns are complete: no segment ends mid-sentence with its continuation starting the next segment, and short answers, acknowledgements, and question responses stay as their own turns when context supports them.
- Include speech present in only one version when it is plausibly real speech the other system dropped. Place it where that version puts it.
- When A and B contain the same underlying speech with different speaker labels, timestamps, punctuation, or wording, output ONE consensus copy of that speech. Never output A's copy followed by B's copy.
- When speaker numbers differ between A and B, they are model-local guesses, not stable identities. Choose the label that best preserves local speaker continuity with the context. Do not invent speaker names unless real names already appear in the input.
- If both versions are messy or resolving them needs audio judgment, return the safer existing segmentation unchanged.

Hard guardrails:
- This is segmentation arbitration, not copyediting. Do NOT paraphrase, summarize, fix grammar, remove fillers, change punctuation style, or compose a polished third version. On wording disagreement, pick the reading from A or B verbatim; never output words absent from both versions.
- The consensus must stay close to version A or version B: mostly one version's text, plus at most small spans borrowed from the other (a dropped short turn, moved boundary words). Wholesale blending of both versions is not allowed.
- Timestamps: copy each output timestamp verbatim from the version whose segment you selected. Never invent, reformat, or average timestamps.
- Speaker attribution changes need overwhelming transcript context. You have no audio; do not infer a speaker from topic or style alone.

Example — same speech, different speaker numbers, output once:
Version A:
  Speaker 0  59:30
  Fair enough. I see a question from the audience.
  Speaker 7  59:43
...
Example — short answer kept as its own turn:
Version A:
  Speaker 0  3:00
  How do you know that withdrawing from the pain is what causes?
  Speaker 1  3:05
...
Version B:
  Speaker 0  3:00
  How do you know that withdrawing from the pain is what causes? Isn't yeah. Yes. How do you know that withdrawing from the pain isn't what causes the suffering
Safe consensus: version A unchanged — it keeps the answer echo as its own turn.

Example — speech only one version caught:
Version A:
  Speaker 0  12:10
  We should check with the fire marshal before the drill.
Version B:
  Speaker 0  12:10
  We should check with the fire marshal before the drill.
  Speaker 1  12:15
  I already emailed him.
Safe consensus: version B — the reply is plausibly real speech version A dropped.

Return via the function call."""
PROMPT_DENOVO_DUAL_V4 = """You compare two ASR segmentations of the same short transcript span. Choose which EXISTING version, A or B, has the better speaker segmentation.

You are a selector, not an editor. Return only the selected version through the function call. Do not return transcript text.

Selection rules:
- Choose a different version only for a substantial speaker-segmentation difference: an existing meaningful speaker turn, a clearly better existing boundary, or a clearly better existing speaker attribution.
- Never create a new segment or speaker turn. A boundary is eligible only if it already exists in the selected version.
- A brief acknowledgement such as "yeah", "yes", "no", "yep", "right", or "okay" is not by itself proof of a separate speaker. However, when one input already contains that separate turn, it is an eligible existing segmentation: use the surrounding turn-taking context to decide whether that input is more coherent. Never extract such words from inside an unsplit input to create a new turn.
- If neither version has a split at a location, there is no split to add.
- Do not choose based on grammar, punctuation, fluency, wording, filler removal, or copyediting quality.
- Speaker numbers can differ between ASR systems. Judge only whether each version's existing turn pattern is internally coherent with the read-only context.
- When the segmentation difference is minor, ambiguous, or requires audio judgment, choose version B as the conservative base.

The application will copy the selected version verbatim, including its exact words, punctuation, timestamps, speaker labels, and boundaries."""
TOOLS_SELECT_TRANSCRIPT_VERSION = [
    {
        "type": "function",
        "function": {
            "name": "select_transcript_version",
            "description": "Select the existing transcript version with better speaker segmentation.",
            "strict": True,
            "parameters": {
                "type": "object",
                "properties": {
...
TOOLS_CORRECT_TRANSCRIPT_SEGMENTS = [
    {
        "type": "function",
        "function": {
            "name": "correct_transcript_segments",
            "description": "Return the corrected or consensus speaker segment list.",
            "strict": True,
            "parameters": {
                "type": "object",
                "properties": {
                    "segments": {
                        ...
                        "items": {
                            ...
                            "required": ["speaker", "timestamp", "dialogue"],
                            "additionalProperties": False,
                        },
                    }
                },
...
def segments_to_block_text(segments, label=None):
    """Format segment dicts as markdown block text for LLM input."""
    lines = []
    if label:
        lines.append(f"### {label}")
        lines.append("")
    for seg in segments:
        speaker = seg.get("speaker_full") or seg.get("speaker") or seg.get("speaker_name") or "Speaker 0"
        ...
        lines.append("")
    return "\n".join(lines).strip()
def validate_transcript_segments_response(parsed, input_count, min_ratio=0.5, max_ratio=1.5, allowed_timestamps=None, required_timestamps=None, min_timestamp_coverage=0.9):
    """
    Validate structured LLM segment response.

    :param allowed_timestamps: optional set; every output timestamp must be in it
        (prompts require timestamps be copied verbatim from the input)
    :param required_timestamps: optional set; at least min_timestamp_coverage of them
        must appear in the output (guards against the LLM silently dropping/merging segments)
    """
    if not parsed or not isinstance(parsed, dict):
        return False, "missing parsed dict"
    segments = parsed.get("segments")
    if not isinstance(segments, list) or len(segments) == 0:
        return False, "empty segments list"
    if input_count > 0:
        ratio = len(segments) / input_count
        if ratio < min_ratio or ratio > max_ratio:
            return False, f"segment count ratio {ratio:.2f} out of range"
    for i, seg in enumerate(segments):
        if not isinstance(seg, dict):
            return False, f"segment {i} not dict"
        for key in ("speaker", "timestamp", "dialogue"):
            if not seg.get(key):
                return False, f"segment {i} missing {key}"
    if allowed_timestamps is not None:
        invented = [seg["timestamp"] for seg in segments if seg["timestamp"] not in allowed_timestamps]
        if invented:
            return False, f"invented timestamps not in input: {invented[:5]}"
    if required_timestamps:
        present = {seg["timestamp"] for seg in segments}
        covered = len(required_timestamps & present) / len(required_timestamps)
        if covered < min_timestamp_coverage:
            return False, f"timestamp coverage {covered:.2f} below {min_timestamp_coverage} (segments dropped)"
    return True, None
def llm_correct_transcript_segments(segments, system_prompt, model, provider="openai", context_before=None, context_after=None, max_retries=3, verbose=False):
    """
    Structured LLM call returning validated corrected segments.

    :return: list of segment dicts or None on failure
    """
    content_parts = []
    if context_before:
        content_parts.append("### context before\n\n" + segments_to_block_text(context_before))
    content_parts.append("### segments to correct\n\n" + segments_to_block_text(segments))
    if context_after:
        content_parts.append("### context after\n\n" + segments_to_block_text(context_after))
    from core.transcript_eval import calc_lev_dist_ratio, normalize_dialogue

    content = "\n\n".join(content_parts)
    tools = TOOLS_CORRECT_TRANSCRIPT_SEGMENTS
    input_timestamps = {seg.get("timestamp") for seg in segments if seg.get("timestamp")}
    context_timestamps = {seg.get("timestamp") for seg in (context_before or []) + (context_after or []) if seg.get("timestamp")}
    input_norm_text = " ".join(normalize_dialogue(seg.get("dialogue"), None) for seg in segments).strip()
    last_error = None
    for attempt in range(max_retries):
        if provider == "anthropic":
            response = anthropic_function_call(system_prompt, content, convert_tools_to_anthropic_format(tools), model=model)
        else:
            response = openai_function_call(system_prompt, content, tools, model=model, verbose=verbose)
        parsed = parse_function_call_response(response, provider=provider)
        ok, err = validate_transcript_segments_response(
            parsed, len(segments),
            allowed_timestamps=input_timestamps | context_timestamps,
            required_timestamps=input_timestamps,
            min_timestamp_coverage=1.0,
        )
        if ok:
            # Drop leading/trailing context echoes (timestamps from the read-only context
            # blocks, not from the input) before checking conservation.
            result_segments = list(parsed["segments"])
            echo_only = context_timestamps - input_timestamps
            while result_segments and result_segments[0].get("timestamp") in echo_only:
                result_segments.pop(0)
            while result_segments and result_segments[-1].get("timestamp") in echo_only:
                result_segments.pop()
            # Word conservation: segmentation repair only moves words between segments, so
            # the concatenated normalized dialogue must be (nearly) unchanged. Rejects
            # paraphrase, dropped dialogue, and echoed context that slipped past filters.
            output_norm_text = " ".join(
                normalize_dialogue(seg.get("dialogue"), None) for seg in result_segments).strip()
            conservation = calc_lev_dist_ratio(input_norm_text, output_norm_text)
            if conservation < 0.995:
                ok = False
                err = f"word conservation {conservation:.4f} below 0.995 (dialogue text changed)"
        if ok:
            return result_segments
        last_error = err
        if verbose:
            print(f"llm_correct_transcript_segments attempt {attempt + 1} failed: {err}")
    if verbose:
        print(f"llm_correct_transcript_segments failed after {max_retries} retries: {last_error}")
    return None
def llm_arbitrate_dual_island(segments_a, segments_b, system_prompt, model, provider="openai", max_retries=3, verbose=False, usage_accumulator=None):
    """
    Structured LLM arbitration for a dual-transcript island.

    :return: list of consensus segment dicts or None on failure
    """
    input_count = max(len(segments_a), len(segments_b), 1)
    allowed_timestamps = {seg.get("timestamp") for seg in list(segments_a) + list(segments_b) if seg.get("timestamp")}
    content = (
        "### version A\n\n" + segments_to_block_text(segments_a)
        + "\n\n### version B\n\n" + segments_to_block_text(segments_b)
    )
    tools = TOOLS_CORRECT_TRANSCRIPT_SEGMENTS
    last_error = None
    for attempt in range(max_retries):
        is_retry = attempt > 0
        if provider == "anthropic":
            response, usage = anthropic_function_call_with_usage(
                system_prompt, content, convert_tools_to_anthropic_format(tools), model=model, verbose=verbose)
            ...
            message, usage, _ = openai_function_call_with_usage(
                system_prompt, content, tools, model=model, verbose=verbose)
            if usage_accumulator:
                usage_accumulator.add_usage(
                    usage['input_tokens'], usage['output_tokens'], usage.get('cached_input_tokens', 0),
                    is_retry=is_retry,
                )
            parsed = parse_function_call_response(message, provider=provider)
        ok, err = validate_transcript_segments_response(parsed, input_count, allowed_timestamps=allowed_timestamps)
        if ok:
            return parsed["segments"]
        last_error = err
        if verbose:
            print(f"llm_arbitrate_dual_island attempt {attempt + 1} failed: {err}")
    if verbose:
        print(f"llm_arbitrate_dual_island failed after {max_retries} retries: {last_error}")
    return None
def llm_arbitrate_dual_chunk(segments_a, segments_b, system_prompt, model, provider="openai", context_before=None, context_after=None, max_retries=3, conservation_min=0.9, verbose=False, usage_accumulator=None):
    """
    Structured LLM arbitration for one word-anchored dual-transcript disagreement chunk.

    Unlike the island path, output is conservation-guarded: the concatenated normalized
    dialogue must stay >= conservation_min similar to version A or version B, which
    rejects paraphrase, duplicated A+B copies, and wholesale rewrites.
    :return: list of consensus segment dicts or None on failure
    """
    from core.transcript_eval import calc_lev_dist_ratio, normalize_dialogue

    input_count = max(len(segments_a), len(segments_b), 1)
    # The sides may legitimately disagree on segment count (blips, merged turns), so the
    # lower count bound scales from the smaller side instead of the fixed 0.5 ratio.
    min_count_ratio = max(0.5 * min(len(segments_a), len(segments_b), input_count) / input_count, 1.0 / (input_count + 1))
    chunk_timestamps = {seg.get("timestamp") for seg in list(segments_a) + list(segments_b) if seg.get("timestamp")}
    context_segments = list(context_before or []) + list(context_after or [])
    context_timestamps = {seg.get("timestamp") for seg in context_segments if seg.get("timestamp")}
    content_parts = []
    if context_before:
        content_parts.append("### agreed context before (read-only)\n\n" + segments_to_block_text(context_before))
    content_parts.append("### version A\n\n" + segments_to_block_text(segments_a))
    content_parts.append("### version B\n\n" + segments_to_block_text(segments_b))
    if context_after:
        content_parts.append("### agreed context after (read-only)\n\n" + segments_to_block_text(context_after))
    content = "\n\n".join(content_parts)
    norm_a = " ".join(normalize_dialogue(seg.get("dialogue"), None) for seg in segments_a).strip()
    norm_b = " ".join(normalize_dialogue(seg.get("dialogue"), None) for seg in segments_b).strip()
    tools = TOOLS_CORRECT_TRANSCRIPT_SEGMENTS
    last_error = None
    for attempt in range(max_retries):
        is_retry = attempt > 0
        ...
        parsed = parse_function_call_response(response, provider=provider)
        ok, err = validate_transcript_segments_response(
            parsed, input_count, min_ratio=min_count_ratio,
            allowed_timestamps=chunk_timestamps | context_timestamps)
        if ok:
            # Drop leading/trailing context echoes (timestamps only in the read-only blocks).
            result_segments = list(parsed["segments"])
            echo_only = context_timestamps - chunk_timestamps
            while result_segments and result_segments[0].get("timestamp") in echo_only:
                result_segments.pop(0)
            while result_segments and result_segments[-1].get("timestamp") in echo_only:
                result_segments.pop()
            if not result_segments:
                ok, err = False, "output contained only context echoes"
        if ok:
            output_norm_text = " ".join(
                normalize_dialogue(seg.get("dialogue"), None) for seg in result_segments).strip()
            conservation = max(
                calc_lev_dist_ratio(output_norm_text, norm_a),
                calc_lev_dist_ratio(output_norm_text, norm_b),
            )
            if conservation < conservation_min:
                ok = False
                err = f"dual word conservation {conservation:.3f} below {conservation_min} (output strays from both versions)"
        if ok:
            return result_segments
        last_error = err
        if verbose:
            print(f"llm_arbitrate_dual_chunk attempt {attempt + 1} failed: {err}")
    if verbose:
        print(f"llm_arbitrate_dual_chunk failed after {max_retries} retries: {last_error}")
    return None
def llm_select_dual_chunk_side(segments_a, segments_b, system_prompt, model, provider="openai", context_before=None, context_after=None, max_retries=3, verbose=False, usage_accumulator=None):
    """Return ``a`` or ``b`` for a disputed chunk without generating transcript text."""
    content_parts = []
    if context_before:
        content_parts.append("### agreed context before (read-only)\n\n" + segments_to_block_text(context_before))
    content_parts.append("### version A\n\n" + segments_to_block_text(segments_a))
    ...
    for attempt in range(max_retries):
        ...
        if provider == "anthropic":
            response, usage = anthropic_function_call_with_usage(
                system_prompt, content, convert_tools_to_anthropic_format(TOOLS_SELECT_TRANSCRIPT_VERSION),
                model=model, verbose=verbose)
        else:
            response, usage, _ = openai_function_call_with_usage(
                system_prompt, content, TOOLS_SELECT_TRANSCRIPT_VERSION, model=model, verbose=verbose)
        if usage_accumulator:
            usage_accumulator.add_usage(
                usage["input_tokens"], usage["output_tokens"], usage.get("cached_input_tokens", 0),
                is_retry=is_retry,
        ...
        parsed = parse_function_call_response(response, provider=provider)
        selected = parsed.get("selected_version") if isinstance(parsed, dict) else None
        if selected in {"a", "b"}:
            return selected
        if verbose:
            print(f"llm_select_dual_chunk_side attempt {attempt + 1} failed: invalid selection {selected!r}")
    return None
def chunk_segments_for_llm(segments, token_cap=1000, adjacent_context=2):
    """
    Split segments into LLM chunks with cross-chunk context snippets.

    :return: list of dicts with keys segments, context_before, context_after
    """
    if not segments:
        return []
    word_cap = token_cap / 0.75
    ...
    while i < n:
        ...
        while i < n:
            seg = segments[i]
            dialogue = seg.get("dialogue") or ""
            words_in_seg = len(dialogue.split()) + 5
            if chunk_segs and word_count + words_in_seg > word_cap:
                break
            chunk_segs.append(seg)
            word_count += words_in_seg
            i += 1
            if words_in_seg > word_cap:
                break
        start_idx = i - len(chunk_segs)
        ctx_before = segments[max(0, start_idx - adjacent_context):start_idx]
        ctx_after = segments[i:i + adjacent_context]
        chunks.append({
            "segments": chunk_segs,
